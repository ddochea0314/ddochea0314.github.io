...
생성하게 됩니다. 현재 제 블로그의 각 포스트는 숫자 ID 단위로 Path를 지정했기 때문에 [ddochea0314.github.io/1](https://ddochea0314.github.io/1) 포스트에 댓글을 달면 아래와 같이 이슈가 생성됩니다.
...
```javascript
  mounted() {
    ...
    const attrs = {
      ...
      repo : '[블로그 리포지토리]',
      "issue-term": `(${this.$page.post.date}) - ${this.$page.post.title}`, // 설정한 graphQL에 맞게 편집가능. 단, 포스트마다 구분되어야함.
      theme : "github-light",
